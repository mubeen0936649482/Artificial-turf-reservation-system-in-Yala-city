...
if (isset($_GET['confirm_id'])) {
    ...
    $sql_confirm = "UPDATE bookings SET status='confirmed' WHERE id='$confirm_id'";
    if ($conn->query($sql_confirm) === TRUE) {
        echo "<div class='success-message'>Booking confirmed successfully</div>";
    } else {
        echo "<div class='error-message'>Error confirming booking: " . $conn->error . "</div>";
    }

    // เปลี่ยนเส้นทางกลับไปยังหน้าหลัก
    header("Location: 1.php"); // หรือใส่หน้าหลักของคุณ
    exit;
}
