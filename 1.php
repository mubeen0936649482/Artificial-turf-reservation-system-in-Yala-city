...
if (isset($_POST['update'])) {
    ...
    // ตรวจสอบว่ามีการจองที่ซ้ำในช่วงเวลาหรือไม่
    $check_sql = "SELECT * FROM bookings WHERE field='$field' AND id != '$edit_id' AND (start_time < '$end_time' AND end_time > '$start_time')";
    $check_result = $conn->query($check_sql);

    if ($check_result->num_rows > 0) {
        echo "<div class='error-message'>Error: มีการจองในช่วงเวลานี้แล้ว</div>";
    } else {
        $sql_update = "UPDATE bookings SET fullname='$fullname', phone='$phone', field='$field', total_price='$total_price' WHERE id='$edit_id'";
...
<body>
    <div class="container">
        ...
        <table>
            ...
            if ($result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td>" . ($status == 'confirmed' ? "<span style='color:green; font-weight:bold;'>Confirmed</span>" : "<a href='confirm_booking.php?confirm_id=" . $row["id"] . "' class='btn btn-small'>Confirm</a>") . "</td>";
        ...
        if (isset($_GET['edit_id'])) {
            ...
            $sql_edit = "SELECT * FROM bookings WHERE id='$edit_id'";
            ...
            if ($result_edit->num_rows > 0) {
                ...
                echo "<form method='POST' action=''>
                    ...
                    <input type='text' name='fullname' value='" . $row_edit['fullname'] . "'><br>
                    <label>Phone:</label>
                    <input type='text' name='phone' value='" . $row_edit['phone'] . "'><br>
                    <label>Field:</label>
                    <input type='text' name='field' value='" . $row_edit['field'] . "'><br>
                    <label>Total Price:</label>
                    <input type='number' name='total_price' value='" . $row_edit['total_price'] . "'><br>
